...
public class PointsAfterRound extends Application {
    ...
    public void start(Stage PointStage)  {
        ...
        for (int i = 0; i < GameMaster.PromtsThisRound.length; i++) {
            ...
            Label writtenbyPlayername = new Label("");
            Label theyVotedFoIt = new Label("");
            Label hegotpoints = new Label("");


            PointGrid.add(promtok,0,i);
            PointGrid.add(writtenbyPlayername,1,i);
            PointGrid.add(theyVotedFoIt,2,i);
            PointGrid.add(hegotpoints,3,i);
